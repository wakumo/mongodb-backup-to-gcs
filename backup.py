...
def backup_mongodb_to_gcs():
    ...
    with tempfile.NamedTemporaryFile(suffix=".gz", delete=True) as tmpfile:
        ...
        dump_cmd = f"mongodump --host={MONGODB_HOST} --archive={backup_filepath} --gzip --quiet"
        print(f"Execute: {dump_cmd}")
...
def prune_bucket(days=7, minimum_backups=7):
    print(f"Prune bucket days:{days} minimum_backups:{minimum_backups}")
    storage_client = get_storage_client()
    bucket = storage_client.bucket(GCS_BUCKET_NAME)
    blobs = list(bucket.list_blobs())
    if len(blobs) <= minimum_backups:
        print(f"Do not prune if not enough backups {len(blobs)}/{minimum_backups}")
        return
...
def upload_to_gcs(file_path, name):
    print(f"Upload to GCS {file_path} {name}")
    storage_client = get_storage_client()
    bucket = storage_client.bucket(GCS_BUCKET_NAME)
    ...
    print(f"A file {file_path} has been uploaded to {GCS_BUCKET_NAME}.")
